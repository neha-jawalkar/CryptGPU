...
class DistributedCommunicator(Communicator):
    ...
    def __init__(self, init_ttp=False):
        ...
        if not dist.is_initialized():
            ...
            if self.world_size == 3:
                self.group01 = dist.new_group([0, 1])
                self.group12 = dist.new_group([1, 2])
                self.group20 = dist.new_group([2, 0])
                
                # Name duplication. Easier for other codes to reference.  
                self.group10 = self.group01
                self.group21 = self.group12
                self.group02 = self.group20
    ...
    def isend(self, tensor, dst, group=None):
        ...
        if group is not None:
            req = dist.broadcast(tensor.data, src=dist.get_rank(), group=group, async_op=True)
            return req
    ...
    def irecv(self, tensor, src=None, group=None):
        ...
        if group is not None:
            req = dist.broadcast(tensor.data, src=src, group=group, async_op=True)
            return req
    ...
    def print_total_communication(self):
        if self.get_rank() in [1,2]:
            self.send_obj(self.comm_bytes, dst=0)
        if self.get_rank() == 0:
            cb0 = self.recv_obj(src=1)
            cb1 = self.recv_obj(src=2)
            self.total_comm_bytes = cb0 + cb1 + self.comm_bytes

            # logging.info("====Total Communication Stats====")
            # logging.info("Rounds: {}".format(self.comm_rounds))
            # logging.info("Total Bytes: {}".format(self.total_comm_bytes))
