...
class Module:
    ...
    def update_parameters(self, learning_rate, grad_threshold=None):
    ...
    def encrypt(self, mode=True, src=0):
        ...
        if mode != self.encrypted:
            ...
            for name, buffer in self.named_buffers(recurse=False):
                ...
                # if not self.training and name in ['running_mean', 'running_var']:
                #     continue
    ...
    def replicate_parameters(self):
        assert self.encrypted, "can only replicate parameters in encrypted models"
        ...
        # for name, buffer in self._buffers.items():
        #     if hasattr(buffer, "share"):
        #         self._buffers[name].rep_share = resharing.replicate_shares(buffer.share)
        for module in self.modules():
            module.replicate_parameters()
...
class _BatchNorm(Module):
    ...
    def __init__(self, num_features, eps=1e-05, momentum=0.1):
        ...
        self.register_buffer("inv_running_var", getattr(pytorch_module, "running_var"))
    ...
    def forward(self, input):
        return input.batchnorm(
            ...
            inv_running_var=self.inv_running_var,
    ...
    def from_onnx(parameters=None, attributes=None):
        ...
        for key, value in parameters.items():
            if key in ["running_mean", "running_var"]:
                ...
                if key == "running_var":
                    module.set_buffer("inv_running_var", 1.0 / torch.sqrt(value+attributes["epsilon"]))
            else:
                module.set_parameter(key, value)
        return module
